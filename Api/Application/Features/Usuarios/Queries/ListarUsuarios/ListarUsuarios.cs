...
public sealed class ListarUsuariosValidator
{
    public Dictionary<string, string[]> Validate(ListarUsuariosQuery query)
    {
        ...
        if (query.TamanoPagina < 1)
            errors[nameof(query.TamanoPagina)] =
            [
                "El tamano de pagina debe ser mayor o igual a 1.",
            ];
...
public sealed class ListarUsuariosHandler
    : IRequestHandler<ListarUsuariosQuery, ListarUsuariosResult>
{
    private readonly AppDbContext _dbContext;
    private readonly ListarUsuariosValidator _validator;

    public ListarUsuariosHandler(AppDbContext dbContext, ListarUsuariosValidator validator)
    {
    ...
    )
    {
        ...
        var items = usuarios.Select(usuario => usuario.ToDto()).ToList();
